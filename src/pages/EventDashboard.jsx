...
export default function EventDashboard() {
  ...
  const { event }   = useEvent(); // dati evento già fetchati dall'AppLayout

  const [kpi,     setKpi]     = useState(null);
  const [loading, setLoading] = useState(true);
  const [daysLeft, setDaysLeft] = useState(null);

  useEffect(() => { fetchKpi(); }, [eventId]);

  useEffect(() => {
    if (event?.event_date) {
      const diff = Math.ceil((new Date(event.event_date) - new Date()) / 86400000);
      setDaysLeft(diff);
    }
  }, [event]);
  ...
  const countdownLabel = daysLeft > 0 ? "Mancano all'evento"
    : daysLeft === 0 ? "È oggi!"
    : "Evento concluso";
