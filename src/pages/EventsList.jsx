...
export default function EventsList() {
  ...
  const firstName = profile?.full_name?.split(" ")[0] || user?.email?.split("@")[0] || "ciao";
...
function EventCard({ event, onClick, t }) {
  ...
  const statusClass = !event.paid ? "pill-warning"
    : isPast ? "pill-neutral"
    : "pill-success";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`status-pill ${statusClass} flex-shrink-0`}>{statusLabel}</span>
          </div>
      ...
      >
        {[
          { icon: BarChart2, label: t("kpi_budget"), value: "€0" },
          { icon: Users,     label: t("kpi_confirmed"), value: "0" },
          { icon: Calendar,  label: t("kpi_roadmap"), value: "0/0" },
        ].map((kpi, i) => (
          <div
            key={i}
          ...
          >
            <p className="text-xs mb-0.5" style={{ color: "var(--text-tertiary)" }}>{kpi.label}</p>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{kpi.value}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="bottom-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="card-secondary mb-5">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm" style={{ color: "var(--text-secondary)" }}>{t("access_event")}</span>
            <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>€2,99</span>
          </div>
          <div className="flex justify-between items-center mb-3">
            <span className="text-sm" style={{ color: "var(--text-secondary)" }}>{t("vat_included")}</span>
          ...
          >
            <span className="font-semibold" style={{ color: "var(--text-primary)" }}>{t("total_label")}</span>
            <span className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>€2,99</span>
          </div>
        </div>
        ...
        <button className="btn-primary w-full mb-3 py-3 text-base">
